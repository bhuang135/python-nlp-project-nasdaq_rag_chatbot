...
def latest_fact(companyfacts: dict, taxonomy: str, concept: str, unit_priority: list[str] | None = None) -> str:
    ...
    top = items[0]
    return f"{concept}: {safe_text(top.get('val'))} | unit={best_unit} | filed={safe_text(top.get('filed'))} | period_end={safe_text(top.get('end'))}"
...
def build_sec_docs(ticker: str, company: str, cik: int | float | None) -> list[dict]:
    ...
    try:
        ...
        # guard: some companies have empty facts
